...
import hintContext from '../data/prompt-hints.json';
...
export async function catalogVersion(): Promise<string> {
  const digest = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(JSON.stringify([PROMPTS, hintContext])));
