...
- Reviewed alternate names are accepted directly for the same score: for example, **Ship of Theseus** also matches **Theseus's ship** in the ship question. These aliases apply only to their reviewed question.
- The [broader alternate-name audit](data/ALIAS_AUDIT.md) adds common regional spellings, full names, titles and abbreviations across 83 more questions—for example **aubergine**, **NES**, **Albus Dumbledore**, **Word** and **JS**. Names still need to satisfy any letter or spelling rule in the question.
...
The playable bank applies documented corrections to that archive and adds **26 reviewed questions**, for **487 questions** total. New questions use fixed 2025 English Wikipedia readership estimates; historical questions retain their editorial grades. Equivalent reviewed names share a score. The question and answer sheet show which scoring method applies. Reviewed question packs make it possible to add complete, source-backed sets without changing game code; the latest pack covers Apollo Moon landings and the Mercury Seven.
